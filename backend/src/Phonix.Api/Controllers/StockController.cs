...
public record AddStockAccountInput(int ProductId, string Username, string Password, string Plan, int Capacity, int Months);
...
public record StockAccountDto(int Id, int ProductId, string Username, string Plan, int Capacity, int Months,
    bool Disabled, string? AddedBy, DateTime AddedAtUtc, List<StockSlotDto> Slots)
{
    public static StockAccountDto From(StockAccount a) =>
        new(a.Id, a.ProductId, a.Username, a.Plan, a.Capacity, a.Months, a.Disabled, a.AddedBy, a.AddedAtUtc,
...
public class StockController : ControllerBase
{
    ...
    public ActionResult<object> Pull(PullStockInput input)
    {
        ...
        if (_store.GetProduct(unit.ProductId) is { SlotFulfillment: true })
        {
            ...
            if (reservation is not { } res) return BadRequest("هیچ اکانتی با جای خالی پیوسته کافی موجود نیست.");
            return new
            {
                stockAccountId = res.Account.Id,
                content = StockFulfillmentService.BuildSlotDeliveryContent(unit.Name, res.Account, count, res.Slots),
            };
        }
