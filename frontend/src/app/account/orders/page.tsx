              ...
              <div className="mt-3 space-y-2">
                {o.items.map((it) => (
                  <div key={`${it.productId}:${it.plan ?? ""}`} className="flex items-center gap-3">
                    <img loading="lazy" decoding="async" src={it.image || undefined} alt={it.name} className="h-10 w-10 rounded-lg object-cover" />
                          ...
                          selUnits.map((u) => {
                            ...
                            return (
                              <div key={u.id} className="rounded-xl p-4" style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.28)" }}>
                                ...
                                {/* Same for a WireGuard service: its own page at /wg/{token}. */}
                                {u.wireGuard?.token && (
                                  <a
                                    href={`/wg/${u.wireGuard.token}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="mt-3 flex h-11 w-full items-center justify-center gap-2 rounded-xl text-[13px] font-black text-white transition hover:brightness-105"
                                    style={{ background: "var(--ac-btn)" }}
