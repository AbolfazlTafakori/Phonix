      ...
      <div className="grid grid-cols-4 gap-5">
        {reviews.map((r) => (
          <div key={r.name} className="hl-card flex flex-col gap-4 rounded-[20px] p-6">
            <Stars />
            <p className="flex-1 text-[14px] leading-[1.9] text-[var(--hl-ink-2)]">{r.text}</p>
            <div className="flex items-center gap-3 border-t border-[var(--hl-border)] pt-4">
              <span className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-[#ef233c] to-[#ff5a1f] text-[15px] font-black text-white">
                {r.name.charAt(0)}
              </span>
              <div>
                <div className="text-[14px] font-bold text-[var(--hl-ink)]">{r.name}</div>
                <div className="text-[12px] text-[var(--hl-muted)]">مشتری</div>
              </div>
            </div>
          </div>
        ))}
