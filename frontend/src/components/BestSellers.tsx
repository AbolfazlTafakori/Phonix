          ...
          >
            <div className="relative aspect-[3/4]">
              <Img src={product.image}
                alt={product.name}
                className="h-full w-full object-cover transition duration-500 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent" />

              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-1.5 p-4">
                {product.logo ? (
                  <Img src={product.logo} alt={product.name} className="max-h-10 w-auto max-w-[70%] object-contain" sizes="240px" />
                ) : (
