...
export default function BestSellers() {
  return (
    <section className="mx-auto mt-16 max-w-[1320px] px-5 sm:mt-24">
      <SectionHeading title="محصولات پر فروش" />
          ...
          >
            <div className="relative aspect-[3/4]">
              ...
              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-1.5 p-4">
                {product.logo ? (
                  <img src={product.logo} alt={product.name} className="max-h-10 w-auto max-w-[70%] object-contain" />
                ) : (
                  <span className="text-2xl font-extrabold tracking-tight text-[#1db954]">
                    Spotify
                  </span>
                )}
                <span className="font-unna text-[11px] tracking-wide text-white/70">
                  Phoenix Verify
                </span>
              </div>
            </div>
