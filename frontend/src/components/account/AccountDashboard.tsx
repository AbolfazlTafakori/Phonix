...
export default function AccountDashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    // forUser (not list): `list` is the staff-only /orders and /tickets admin endpoint and 403s for a
    // regular customer, which the .catch() swallowed — recent orders/tickets silently stayed empty.
    // Fetch the full list (not pre-sliced) so stats like "open tickets" count the whole account, not
    // just whatever happened to fit in the recent-activity preview.
    api.orders.forUser(user.id).then(setOrders).catch(() => {});
    api.tickets.forUser(user.id).then(setTickets).catch(() => {});
  ...
  // Recent-activity previews show only the 2 latest — the full lists above still back the stats/counts.
