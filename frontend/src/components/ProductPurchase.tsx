...
export default function ProductPurchase({ product }: { product: Product }) {
  ...
  return (
    <div>
      {types.length > 0 && (
        <>
          <div className="mt-6 flex gap-2 rounded-2xl border border-[var(--hl-border)] bg-[var(--hl-border)]/20 p-1.5">
            {types.map((t) => (
              <button
                key={t}
                onClick={() => selectType(t)}
                className={`flex-1 rounded-xl px-4 py-2.5 text-sm font-bold transition ${type === t ? "bg-gradient-to-l from-[#e60053] to-[#9c0038] text-white" : "text-[var(--hl-ink-2)] hover:text-[var(--hl-ink)]"}`}
              >
                {t}
              </button>
            ))}
            ...
            {typedPlans.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border p-4 text-center transition ${active ? "border-[#e60053] bg-[#e60053]/10" : "border-[var(--hl-border)] hover:border-[var(--hl-ink-2)]"}`}
                >
                  <p className="text-sm font-bold text-[var(--hl-ink)]">{toFa(p.months)} ماهه</p>
                  {p.userCount > 0 && (
                    <span className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-[var(--hl-border)]/30 px-2 py-0.5 text-[11px] font-bold text-[var(--hl-ink-2)]">
                  ...
                  {p.discountPercent > 0 && <p className="mt-0.5 text-[11px] text-[var(--hl-muted)] line-through">{formatToman(p.price)}</p>}
                </button>
              );
            })}
          </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[var(--hl-border)] bg-white p-5">
        <div>
          ...
          {planLabel && <p className="mt-1 text-xs text-[var(--hl-muted)]">{planLabel}</p>}
        </div>
        <AddToCart
          key={planId ?? "base"}
